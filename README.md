...
`algoitni`는 알고리즘 입문자가 대표 파이썬 구현 1개를 기준으로 개념 학습, 단계형 연습, 반례 기반 오답 진단까지 이어서 수행할 수 있게 돕는 학습 웹 MVP입니다.

## 프로젝트 소개

- 프로젝트 한 줄 소개: 알고리즘마다 대표 파이썬 구현 1개를 중심으로 학습 흐름을 고정한 docs-first 워크숍 MVP
...
- 이번 단계에서 의도적으로 제외하는 것: 백지 코딩, 로그인/동기화, LLM 기반 정답 판정

## 핵심 기능

- 알고리즘 개념 설명, 복잡도, 대표 실수, 시각화 자료 제공
- 알고리즘별 대표 파이썬 구현 레슨 1개와 `빈칸 -> 파슨스` 단계형 연습
- 정적 힌트와 LLM 보조 기반의 힌트, 재설명, 오답 분석
- 실전 문제 풀이 UI와 반례 후보 생성 보조가 포함된 counterexample 파이프라인
...
- 레슨 화면: 대표 구현 코드, `빈칸 -> 파슨스` 인터랙션, 힌트, 진행 상태 제공
- 실전 문제 화면: 문제 설명, 코드 입력, 제출 결과, 반례 결과 제공

## 아키텍처 요약

- 클라이언트: Flask + Jinja로 렌더링된 HTML에 바닐라 JavaScript와 Tailwind CSS를 결합한다
- 서버: Python Flask가 정적 콘텐츠 로딩, LLM 보조 API, counterexample 파이프라인을 담당한다
...
- LLM 제공자, 샌드박스 제한, validator 세부 구현은 후속 기술 설계에서 고정한다
